...
from parlai.core.utils import padded_3d, warn_once
from parlai.tasks.redial.extract_entities import _text2entities

from .modules import TransformerGeneratorModel, TransformerMemNetModel
...
def add_common_cmdline_args(argparser):
    ...
    argparser.add_argument(
        "--learn-positional-embeddings", type="bool", default=False
    )
...
class TransformerRankerAgent(TorchRankerAgent):
    ...
    def _score(self, output, cands):
        if cands.dim() == 2:
            ...
            return torch.bmm(
                output.unsqueeze(1), cands.transpose(1, 2)
            ).squeeze(1)
        else:
            raise RuntimeError(
    ...
    def vectorize(self, obs, history, **kwargs):
        ...
        if obs.get("label_candidates", None) is None:
            ...
            for entity, is_movie in dbpedia_entities_tuples:
                entity_id = self._entity2id.get(entity)
